...
<div class="py-3">
    <a href="{{ route('news.index', ['lang' => 'vi']) }}" class="btn btn-outline-danger">
        VI
    </a>

    <a href="{{ route('news.index', ['lang' => 'en']) }}" class="btn btn-outline-primary">
        EN
    </a>
</div>
      ...
      <table class="table table-hover text-nowrap">
        ...
        <tbody>
          @foreach ($news as $item)
              <tr>
                    <td>{{ $loop->index + 1 }}</td>
                    <td><a href="{{ route('media.news-detail', $item->id) }}" target="_blank">{{ $item->title }}</a></td>
